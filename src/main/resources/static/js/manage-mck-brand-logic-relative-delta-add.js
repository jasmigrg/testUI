const MCK_RELATIVE_DELTA_FIELD_DEFS = [
  ...
  { field: 'relativeAllowableMinPrice', headerName: 'Relative Allowable Min Price', minWidth: 240, type: 'number' },
  { field: 'relativeAllowableMaxPrice', headerName: 'Relative Allowable Max Price', minWidth: 240, type: 'number' }
];

const MCK_RELATIVE_DELTA_DATE_FIELDS = new Set(['effectiveDate', 'terminationDate']);
const MCK_RELATIVE_DELTA_NUMBER_FIELDS = new Set(['relativeAllowableMinPrice', 'relativeAllowableMaxPrice']);
const MCK_RELATIVE_DELTA_OUTBOUND_FIELDS = [
  { localField: 'terminationDate', backendField: 'terminationDate' },
  { localField: 'relativeAllowableMinPrice', backendField: 'relativeAllowableMinPrice' },
  { localField: 'relativeAllowableMaxPrice', backendField: 'relativeAllowableMaxPrice' },
  { localField: 'itemCategory', backendField: 'itemCategory' },
  { localField: 'effectiveDate', backendField: 'effectiveDate' }
];

const MckBrandLogicRelativeDeltaAddPage = {
  entityName: '',
  gridApi: null,
  gridElement: null,
  ...
  init() {
    this.entityName = String(window.MCK_RELATIVE_DELTA_ENTITY_NAME || 'gm-core-parameter-mckb-relative-price-delta').trim();
    this.cacheDom();
    this.initGrid();
  ...
  cacheDom() {
    ...
    this.uploadStatusInputs = Array.from(document.querySelectorAll('input[name="mckRelativeDeltaUploadStatus"]'));
  ...
  createBlankRow() {
    const row = {};
    MCK_RELATIVE_DELTA_FIELD_DEFS.forEach(({ field }) => {
      row[field] = '';
    });
    row.uploadStatus = '';
    row.uploadErrors = [];
  ...
  initGrid() {
    ...
    this.gridApi = DynamicGrid.createGrid({
      gridElementId: 'mckRelativeDeltaAddGrid',
      ...
      columns: [
        ...
        ...MCK_RELATIVE_DELTA_FIELD_DEFS.map((column) => this.buildColumn(column, dateComparator))
      ]
    });

    this.gridElement = document.getElementById('mckRelativeDeltaAddGrid');
    ...
    if (window.CommunityGridPaste?.attach) {
      const pasteableFields = MCK_RELATIVE_DELTA_FIELD_DEFS.map((column) => column.field);
    ...
    setTimeout(() => {
      if (typeof GridManager !== 'undefined' && this.gridApi) {
        GridManager.init(this.gridApi, 'mckRelativeDeltaAddGrid');
      }
    }, 300);
  },
  ...
  bindToolbarActions() {
    document.addEventListener('click', (event) => {
      ...
      switch (actionButton.dataset.action) {
        case 'back':
          window.location.href = `${window.MCK_BRAND_LOGIC_PAGE_URL || '/manage-mck-brand-logic'}?tab=relative-delta`;
          break;
        case 'delete':
  ...
  getStorageKey() {
    return `mck-relative-delta-jobs:${this.entityName}`;
  },
  ...
  mapErrorField(field) {
    ...
    const match = MCK_RELATIVE_DELTA_FIELD_DEFS.find((column) => this.normalizeHeader(column.headerName) === normalized || this.normalizeHeader(column.field) === normalized);
    return match?.field || '';
  },
  ...
  getFieldValue(source, field) {
    ...
    const column = MCK_RELATIVE_DELTA_FIELD_DEFS.find((item) => item.field === field);
    if (column) {
  ...
  resolveUploadContext() {
    return {
      ...
      programId: String(window.MCK_RELATIVE_DELTA_PROGRAM_ID || 'PROG001'),
      workStationId: String(window.MCK_RELATIVE_DELTA_WORK_STATION_ID || 'WS001')
    };
  },
  ...
  toBackendRecord(row) {
    const payload = {};
    MCK_RELATIVE_DELTA_OUTBOUND_FIELDS.forEach(({ localField, backendField }) => {
  ...
  resolvePasteHeaderField(header, allowedFields = null) {
    ...
    const match = MCK_RELATIVE_DELTA_FIELD_DEFS.find(({ field, headerName }) => (
  ...
  normalizeRow(row) {
    ...
    MCK_RELATIVE_DELTA_FIELD_DEFS.forEach(({ field }) => {
      ...
      if (MCK_RELATIVE_DELTA_DATE_FIELDS.has(field)) {
        normalized[field] = this.toUsDate(value || '');
      } else if (MCK_RELATIVE_DELTA_NUMBER_FIELDS.has(field)) {
  ...
  isRowEmpty(row) {
    return MCK_RELATIVE_DELTA_FIELD_DEFS.every(({ field }) => !String(row[field] || '').trim());
  },
  ...
  showInfo(message, type = 'success') {
    ...
    let container = document.getElementById('mckRelativeDeltaAddPageToastLayer');
    if (!container) {
      container = document.createElement('div');
      container.id = 'mckRelativeDeltaAddPageToastLayer';
...
document.addEventListener('DOMContentLoaded', () => {
  MckBrandLogicRelativeDeltaAddPage.init();
});
