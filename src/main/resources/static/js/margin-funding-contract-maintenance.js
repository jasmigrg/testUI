...
const MarginFundingContractMaintenancePage = {
  ...
  mapColumnToApiField(colId, context = 'filter') {
    const fieldMap = {
      ...
      ie: 'ie',
      effectiveFrom: context === 'sort' ? 'effective_date' : 'effectiveDate',
      effectiveThru: context === 'sort' ? 'termination_date' : 'terminationDate',
  ...
  mapApiRow(row) {
    return {
      ...
      ie: row?.ie ?? '',
      effectiveFrom: this.normalizeDateValueForDisplay(row?.effectiveFrom ?? row?.effectiveDate),
      effectiveThru: this.normalizeDateValueForDisplay(row?.effectiveThru ?? row?.terminationDate),
