class KviInlineSaveCellEditor {
  init(params) {
    ...
    this.eGui.className = 'inline-save-editor';

    this.input = document.createElement('input');
    this.input.type = 'text';
    this.input.className = 'inline-save-input';
    this.input.value = params.value == null ? '' : String(params.value);
    this.input.placeholder = '_';

    this.saveBtn = document.createElement('button');
    this.saveBtn.type = 'button';
    this.saveBtn.className = 'inline-save-btn';
...
const KviRecommendationLogicAddPage = {
  ...
  toastMessageDefaults: {
    ...
    INIT_BATCH_ERROR: 'Failed to initialize batch section.',
  ...
  initBulkUploadFlow() {
    ...
    this.bulkFlow.init().catch((error) => {
      ...
      this.showInfo(this.resolveToastMessage(error?.payload || error, 'INIT_BATCH_ERROR'), 'error');
    });
  },
  ...
  validationCellRules(field) {
    return {
      'grid-cell-error': (params) => Array.isArray(params.data?.uploadErrors) && params.data.uploadErrors.includes(field)
    };
  },
  ...
  async fetchJson(endpoint, options = {}) {
    ...
    if (!response.ok) {
      const message = this.resolveToastMessage(payload, 'LOAD_ERROR');
  ...
  async loadBatchData(batchId) {
    ...
    try {
      ...
      if (this.uploadStatusRow) this.uploadStatusRow.hidden = rows.length === 0;
      this.applyUploadFilter();
      this.showInfo(this.resolveToastMessage({ message: `Loaded batch ${batchId} (${rows.length} row(s)).` }, 'BATCH_ROWS_LOAD_SUCCESS'), 'success');
    } catch (error) {
      console.error('Failed to load batch rows:', error);
      this.showInfo(this.resolveToastMessage(error?.payload || error, 'BATCH_ROWS_LOAD_ERROR'), 'error');
    }
  },
  ...
  async deleteBatchRow(batchId) {
    ...
    if (this.shouldUseMockBulkUpload()) {
      ...
      try {
        ...
        this.showInfo(this.resolveToastMessage(error?.payload || error, 'BATCH_DELETE_ERROR'), 'error');
        return;
      }
    }
    ...
    this.showInfo(this.resolveToastMessage(null, 'BATCH_DELETE_SUCCESS'), 'success');
  },
  ...
  async createMockBatchFromFile(file) {
    ...
    this.showInfo(
      this.resolveToastMessage({ message: `Batch ${batchNumber} created. Click batch number to load rows.` }, 'BATCH_CREATE_SUCCESS'),
      'success'
    );
    return true;
  },
  ...
  async processUploadedCsv(file) {
    if (this.bulkFlow?.handleFileUpload) {
      try {
        ...
        this.showInfo(this.resolveToastMessage(error?.payload || error, 'PROCESS_UPLOAD_ERROR'), 'error');
        return false;
      }
    }
    ...
    try {
      ...
      await this.loadBatchRows();
      this.showInfo(
        this.resolveToastMessage({ message: `Batch ${createdBatch.batchNumber} created. Import started.` }, 'BATCH_CREATE_SUCCESS'),
        'success'
      );
      return true;
    } catch (error) {
  ...
  resolveToastMessage(payload, fallbackKey) {
    if (window.PageToast?.resolveMessage) {
      return window.PageToast.resolveMessage(payload, fallbackKey, this.toastMessageDefaults);
    }
    const fallback = this.toastMessageDefaults[fallbackKey] || 'Something went wrong.';
