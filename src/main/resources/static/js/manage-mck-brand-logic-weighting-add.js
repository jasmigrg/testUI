const MCK_WEIGHTING_FIELD_DEFS = [
  ...
  { field: 'itemCategory', headerName: 'Item Category', minWidth: 190 },
  { field: 'itemNum', headerName: 'Item Num', minWidth: 170, type: 'number' },
  { field: 'relativeProfitabilityWeighting', headerName: 'Relative Profitability Weighting', minWidth: 260, type: 'number' },
  { field: 'relativeShareWeighting', headerName: 'Relative Share Weighting', minWidth: 220, type: 'number' },
  { field: 'relativeQualityWeighting', headerName: 'Relative Quality Weighting', minWidth: 220, type: 'number' }
];

const MCK_WEIGHTING_DATE_FIELDS = new Set(['effectiveDate', 'terminationDate']);
const MCK_WEIGHTING_NUMBER_FIELDS = new Set([
  'itemNum',
...
const MCK_WEIGHTING_OUTBOUND_FIELDS = [
  { localField: 'terminationDate', backendField: 'terminationDate' },
  { localField: 'itemCategory', backendField: 'itemCategory' },
  { localField: 'itemNum', backendField: 'itemNum' },
  { localField: 'relativeProfitabilityWeighting', backendField: 'relativeProfitabilityWeighting' },
  { localField: 'relativeShareWeighting', backendField: 'relativeShareWeighting' },
  { localField: 'effectiveDate', backendField: 'effectiveDate' },
  { localField: 'relativeQualityWeighting', backendField: 'relativeQualityWeighting' }
];

const MckBrandLogicWeightingAddPage = {
  entityName: '',
  gridApi: null,
  gridElement: null,
  ...
  init() {
    this.entityName = String(window.MCK_WEIGHTING_ENTITY_NAME || 'mckb-split-price-scoring-weighting').trim();
    this.cacheDom();
    this.initGrid();
  ...
  cacheDom() {
    ...
    this.uploadStatusInputs = Array.from(document.querySelectorAll('input[name="mckWeightingUploadStatus"]'));
  ...
  createBlankRow() {
    const row = {};
    MCK_WEIGHTING_FIELD_DEFS.forEach(({ field }) => {
      row[field] = '';
    });
    row.uploadStatus = '';
    row.uploadErrors = [];
  ...
  initGrid() {
    ...
    this.gridApi = DynamicGrid.createGrid({
      gridElementId: 'mckWeightingAddGrid',
      ...
      columns: [
        ...
        ...MCK_WEIGHTING_FIELD_DEFS.map((column) => this.buildColumn(column, dateComparator))
      ]
    });

    this.gridElement = document.getElementById('mckWeightingAddGrid');
    ...
    if (window.CommunityGridPaste?.attach) {
      const pasteableFields = MCK_WEIGHTING_FIELD_DEFS.map((column) => column.field);
    ...
    setTimeout(() => {
      if (typeof GridManager !== 'undefined' && this.gridApi) {
        GridManager.init(this.gridApi, 'mckWeightingAddGrid');
      }
    }, 300);
  },
  ...
  bindToolbarActions() {
    document.addEventListener('click', (event) => {
      ...
      switch (actionButton.dataset.action) {
        case 'back':
          window.location.href = window.MCK_BRAND_LOGIC_PAGE_URL || '/manage-mck-brand-logic';
          break;
        case 'delete':
  ...
  initViewActions() {
    ...
    GridToolbar.bindDensityControls({
      ...
      gridElement: this.gridElement,
      defaultMode: 'compact',
      densityClassPrefix: 'mck-density'
    });
  },
  ...
  getStorageKey() {
    return `mck-weighting-jobs:${this.entityName}`;
  },
  ...
  mapErrorField(field) {
    ...
    const match = MCK_WEIGHTING_FIELD_DEFS.find((column) => this.normalizeHeader(column.headerName) === normalized || this.normalizeHeader(column.field) === normalized);
    return match?.field || '';
  },
  ...
  getFieldValue(source, field) {
    ...
    const column = MCK_WEIGHTING_FIELD_DEFS.find((item) => item.field === field);
    if (column) {
  ...
  resolveUploadContext() {
    return {
      ...
      programId: String(window.MCK_WEIGHTING_PROGRAM_ID || 'PROG001'),
      workStationId: String(window.MCK_WEIGHTING_WORK_STATION_ID || 'WS001')
    };
  },
  ...
  toBackendRecord(row) {
    const payload = {};
    MCK_WEIGHTING_OUTBOUND_FIELDS.forEach(({ localField, backendField }) => {
  ...
  resolvePasteHeaderField(header, allowedFields = null) {
    ...
    const match = MCK_WEIGHTING_FIELD_DEFS.find(({ field, headerName }) => (
  ...
  normalizeRow(row) {
    ...
    MCK_WEIGHTING_FIELD_DEFS.forEach(({ field }) => {
      ...
      if (MCK_WEIGHTING_DATE_FIELDS.has(field)) {
        normalized[field] = this.toUsDate(value || '');
      } else if (MCK_WEIGHTING_NUMBER_FIELDS.has(field)) {
  ...
  isRowEmpty(row) {
    return MCK_WEIGHTING_FIELD_DEFS.every(({ field }) => !String(row[field] || '').trim());
  },
  ...
  applyUploadFilter() {
    const sourceRows = Array.isArray(this.uploadedRows) ? this.uploadedRows : [];
    let rows = sourceRows;
    if (this.uploadFilter === 'success') {
      rows = sourceRows.filter((row) => this.isSuccessfulUploadRow(row));
    } else if (this.uploadFilter === 'error') {
      rows = sourceRows.filter((row) => !this.isSuccessfulUploadRow(row));
    }
    this.gridApi?.setGridOption?.('rowData', rows.length > 0 ? rows : this.selectedJobId ? [] : this.initialRows());
    this.gridApi?.refreshCells?.({ force: true });
  },

  clearColumnFilters() {
    if (!this.gridApi?.setFilterModel) return;
  ...
  saveDraft() {
    this.syncUploadedRowsFromGrid(true);
    this.showInfo('Draft saved locally.', 'success');
  },
  ...
  showInfo(message, type = 'success') {
    ...
    let container = document.getElementById('mckWeightingAddPageToastLayer');
    if (!container) {
      container = document.createElement('div');
      container.id = 'mckWeightingAddPageToastLayer';
...
document.addEventListener('DOMContentLoaded', () => {
  MckBrandLogicWeightingAddPage.init();
});
