const MFC_FIELD_DEFS = [
  ...
  { field: 'szPriceFormula', headerName: 'Price Formula', minWidth: 180 },
  { field: 'percentageBeneficiary', headerName: 'Percent', minWidth: 140, type: 'number' },
...
const MFC_NUMBER_FIELDS = new Set(['recordId', 'vendorFamilyNo', 'percentageBeneficiary']);

const MFC_HEADER_ALIASES = {
  recordId: ['vendorprogram', 'recordid'],
  vendorFamilyNo: ['vendorfamilynumber', 'vendorfamilyno'],
  szPriceFormula: ['priceformula', 'szpriceformula'],
  percentageBeneficiary: ['percent', 'percentagebeneficiary'],
...
const MFC_OUTBOUND_FIELDS = [
  ...
  { localField: 'szPriceFormula', backendField: 'szPriceFormula' },
  { localField: 'percentageBeneficiary', backendField: 'percentageBeneficiary' },
...
const MarginFundingPriceMaintenanceAddPage = {
  entityName: '',
  gridApi: null,
  gridElement: null,
  ...
  init() {
    this.entityName = String(
      window.MFP_ENTITY_NAME || 'distribution-fee-pricing-attribute'
    ).trim();
    this.cacheDom();
    this.initGrid();
  ...
  initGrid() {
    ...
    this.gridApi = DynamicGrid.createGrid({
      gridElementId: 'mfpPriceMaintenanceAddGrid',
    ...
    this.gridElement = document.getElementById('mfpPriceMaintenanceAddGrid');
    ...
    setTimeout(() => {
      if (typeof GridManager !== 'undefined' && this.gridApi) {
        GridManager.init(this.gridApi, 'mfpPriceMaintenanceAddGrid');
      }
    }, 300);
  },
  ...
  bindToolbarActions() {
    document.addEventListener('click', (event) => {
      ...
      switch (actionButton.dataset.action) {
        case 'back':
          window.location.href = window.MFP_LIST_PAGE_URL || '/margin-funding-price-maintenance';
          break;
        case 'delete':
  ...
  getStorageKey() {
    return `margin-funding-price-jobs:${this.entityName}`;
  },
  ...
  resolveUploadContext() {
    return {
      ...
      programId: String(window.MFP_PROGRAM_ID || 'PROG001'),
      workStationId: String(window.MFP_WORK_STATION_ID || 'WS001')
    };
  },
  ...
  ensureToastContainer() {
    let container = document.getElementById('marginFundingPriceMaintenanceAddPageToastLayer');
    if (container) return container;

    container = document.createElement('div');
    container.id = 'marginFundingPriceMaintenanceAddPageToastLayer';
...
document.addEventListener('DOMContentLoaded', () => {
  MarginFundingPriceMaintenanceAddPage.init();
});
