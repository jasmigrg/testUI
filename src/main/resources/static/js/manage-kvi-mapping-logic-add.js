...
const KviMappingLogicAddPage = {
  ...
  initGrid() {
    ...
    this.gridApi = DynamicGrid.createGrid({
      gridElementId: 'kviMappingParameterAddGrid',
      ...
      columns: [
        ...
        },
        {
          ...
          cellEditorSelector: (params) => (this.isErrorCell(params, 'revenuePenetrationPctl')
            ? { component: 'kviInlineSaveCellEditor', params: { pageRef: this } }
            : undefined),
          cellClassRules: this.validationCellRules('revenuePenetrationPctl')
        },
        {
          ...
          cellEditorSelector: (params) => (this.isErrorCell(params, 'prcaPenetrationPctl')
            ? { component: 'kviInlineSaveCellEditor', params: { pageRef: this } }
            : undefined),
          cellClassRules: this.validationCellRules('prcaPenetrationPctl')
        },
        {
          ...
          cellEditorSelector: (params) => (this.isErrorCell(params, 'revenuePenetrationAdj')
            ? { component: 'kviInlineSaveCellEditor', params: { pageRef: this } }
            : undefined),
          cellClassRules: this.validationCellRules('revenuePenetrationAdj')
        },
        {
          ...
          cellEditorSelector: (params) => (this.isErrorCell(params, 'prcaPenetrationAdj')
            ? { component: 'kviInlineSaveCellEditor', params: { pageRef: this } }
            : undefined),
          cellClassRules: this.validationCellRules('prcaPenetrationAdj')
        }
      ]
    });

    this.gridElement = document.getElementById('kviMappingParameterAddGrid');
  ...
  getScreenCode() {
    return String(window.BULK_UPLOAD_SCREEN_CODE || 'KVI_MAPPING_PARAMETER').trim() || 'KVI_MAPPING_PARAMETER';
  },
  ...
  normalizeBatchDataRow(item) {
    const normalized = this.normalizeRow({
      ...
      revenuePenetrationPctl: item.revenuePenetrationPctl || item.revenue_penetration_pctl || '',
      prcaPenetrationPctl: item.prcaPenetrationPctl || item.prca_penetration_pctl || '',
      revenuePenetrationAdj: item.revenuePenetrationAdj || item.revenue_penetration_adj || '',
      prcaPenetrationAdj: item.prcaPenetrationAdj || item.prca_penetration_adj || ''
    });
    ...
    const mappedErrors = rowErrors
      ...
      .map((field) => {
        ...
        if (field === 'revenuepenetrationpctl') return 'revenuePenetrationPctl';
        if (field === 'prcapenetrationpctl') return 'prcaPenetrationPctl';
        if (field === 'revenuepenetrationadj') return 'revenuePenetrationAdj';
        if (field === 'prcapenetrationadj') return 'prcaPenetrationAdj';
        return '';
      })
      .filter(Boolean);
  ...
  applyAdvancedFilters() {
    const kindByField = {
      ...
      revenuePenetrationPctl: 'number',
      prcaPenetrationPctl: 'number',
      revenuePenetrationAdj: 'number',
      prcaPenetrationAdj: 'number'
    };
  ...
  normalizeRow(row) {
    return {
      ...
      revenuePenetrationPctl: String(row.revenuePenetrationPctl || '').trim(),
      prcaPenetrationPctl: String(row.prcaPenetrationPctl || '').trim(),
      revenuePenetrationAdj: String(row.revenuePenetrationAdj || '').trim(),
      prcaPenetrationAdj: String(row.prcaPenetrationAdj || '').trim()
    };
  },
  ...
  isRowEmpty(row) {
    return !row.effectiveDate
      && !row.terminationDate
      && !row.revenuePenetrationPctl
      && !row.prcaPenetrationPctl
      && !row.revenuePenetrationAdj
      && !row.prcaPenetrationAdj;
  },
  ...
  validateRow(row) {
    ...
    if (!this.isValidThreshold(row.revenuePenetrationPctl)) errors.push('revenuePenetrationPctl');
    if (!this.isValidThreshold(row.prcaPenetrationPctl)) errors.push('prcaPenetrationPctl');
    if (!this.isValidThreshold(row.revenuePenetrationAdj)) errors.push('revenuePenetrationAdj');
    if (!this.isValidThreshold(row.prcaPenetrationAdj)) errors.push('prcaPenetrationAdj');
    return {
  ...
  async parseAndMapCsvFile(file) {
    ...
    return parsed.rows.map((cells) => {
      const row = this.normalizeRow({
        ...
        revenuePenetrationPctl: cells[indexByHeader.revenuepenetrationpctl] || '',
        prcaPenetrationPctl: cells[indexByHeader.prcapenetrationpctl] || '',
        revenuePenetrationAdj: cells[indexByHeader.revenuepenetrationadj] || '',
        prcaPenetrationAdj: cells[indexByHeader.prcapenetrationadj] || ''
      });
  ...
  async createMockBatchFromFile(file) {
    ...
    const batchId = `KVI-MAPPING-${Date.now()}`;
    ...
    const batch = {
      ...
      priceRuleLevel: 'KVI_MAPPING',
      startDate: today,
      endDate: today,
      programId: 'KVI_MAPPING',
