const KVI_MAPPING_FIELD_DEFS = [
  ...
  { field: 'revenuePenetrationPctl', headerName: 'Revenue Penetration PCTL', minWidth: 240, type: 'number' },
  { field: 'prcaPenetrationPctl', headerName: 'PRCA Penetration PCTL', minWidth: 220, type: 'number' },
  { field: 'revenuePenetrationAdj', headerName: 'Revenue Penetration ADJ', minWidth: 230, type: 'number' },
  { field: 'prcaPenetrationAdj', headerName: 'PRCA Penetration ADJ', minWidth: 210, type: 'number' }
];

const KVI_MAPPING_DATE_FIELDS = new Set(['effectiveDate', 'terminationDate']);
const KVI_MAPPING_NUMBER_FIELDS = new Set([
...
const KVI_MAPPING_OUTBOUND_FIELDS = [
  { localField: 'effectiveDate', backendField: 'effectiveDate' },
  { localField: 'terminationDate', backendField: 'terminationDate' },
  { localField: 'revenuePenetrationPctl', backendField: 'revenuePenetrationPctl' },
  { localField: 'prcaPenetrationPctl', backendField: 'prcaPenetrationPctl' },
  { localField: 'revenuePenetrationAdj', backendField: 'revenuePenetrationAdj' },
  { localField: 'prcaPenetrationAdj', backendField: 'prcaPenetrationAdj' }
];
...
const KviMappingLogicAddPage = {
  ...
  init() {
    this.entityName = String(window.KVI_MAPPING_ENTITY_NAME || 'kvi-mapping-paramter').trim();
    this.cacheDom();
    this.initGrid();
  ...
  createBlankRow() {
    const row = {};
    KVI_MAPPING_FIELD_DEFS.forEach(({ field }) => {
      row[field] = '';
    });
    row.uploadStatus = '';
    row.uploadErrors = [];
  ...
  initGrid() {
    ...
    this.gridApi = DynamicGrid.createGrid({
      ...
      columns: [
        ...
        ...KVI_MAPPING_FIELD_DEFS.map((column) => this.buildColumn(column, dateComparator))
      ]
    });
    ...
    if (window.CommunityGridPaste?.attach) {
      const pasteableFields = KVI_MAPPING_FIELD_DEFS.map((column) => column.field);
    ...
    setTimeout(() => {
      if (typeof GridManager !== 'undefined' && this.gridApi) {
        GridManager.init(this.gridApi, 'kviMappingParameterAddGrid');
      }
    }, 300);
  },
  ...
  getStorageKey() {
    return `kvi-mapping-jobs:${this.entityName}`;
  },
  ...
  mapErrorField(field) {
    ...
    const match = KVI_MAPPING_FIELD_DEFS.find((column) => this.normalizeHeader(column.headerName) === normalized || this.normalizeHeader(column.field) === normalized);
    return match?.field || '';
  },
  ...
  getFieldValue(source, field) {
    ...
    const column = KVI_MAPPING_FIELD_DEFS.find((item) => item.field === field);
    if (column) {
  ...
  resolveUploadContext() {
    return {
      ...
      programId: String(window.KVI_MAPPING_PROGRAM_ID || 'PROG001'),
      workStationId: String(window.KVI_MAPPING_WORK_STATION_ID || 'WS001')
    };
  },
  ...
  toBackendRecord(row) {
    const payload = {};
    KVI_MAPPING_OUTBOUND_FIELDS.forEach(({ localField, backendField }) => {
  ...
  resolvePasteHeaderField(header, allowedFields = null) {
    ...
    const match = KVI_MAPPING_FIELD_DEFS.find(({ field, headerName }) => (
  ...
  normalizeRow(row) {
    ...
    KVI_MAPPING_FIELD_DEFS.forEach(({ field }) => {
      ...
      if (KVI_MAPPING_DATE_FIELDS.has(field)) {
        normalized[field] = this.toUsDate(value || '');
      } else if (KVI_MAPPING_NUMBER_FIELDS.has(field)) {
  ...
  isRowEmpty(row) {
    return KVI_MAPPING_FIELD_DEFS.every(({ field }) => !String(row[field] || '').trim());
  },
  ...
  async parseAndMapCsvFile(file) {
    ...
    return parsed.rows.map((cells) => {
      ...
      KVI_MAPPING_FIELD_DEFS.forEach(({ field, headerName }) => {
  ...
  applyAdvancedFilters() {
    const fieldTypeMap = {};
    KVI_MAPPING_FIELD_DEFS.forEach((column) => {
