const MFC_FIELD_DEFS = [
  ...
  { field: 'manufacturerContractId', headerName: 'Manufacturer Contract ID', minWidth: 210 },
  { field: 'szGenericStr30A', headerName: 'MFG Contract ID Wildcard Lookup', minWidth: 250 },
  { field: 'contractType', headerName: 'Contract Type', minWidth: 160 },
  { field: 'includeExclude', headerName: 'I/E', minWidth: 110 },
  { field: 'effectiveDate', headerName: 'Effective Date', minWidth: 180, type: 'date' },
  { field: 'terminationDate', headerName: 'Termination Date', minWidth: 180, type: 'date' },
...
const MFC_NUMBER_FIELDS = new Set(['recordId', 'vendorFamilyNo']);

const MFC_HEADER_ALIASES = {
  recordId: ['vendorprogram', 'recordid'],
  terminationDate: ['terminationdate', 'effectivethru'],
  manufacturerContractId: ['manufacturercontractid'],
  workStationId: ['workstationid', 'workstation', 'workstnid'],
  contractType: ['contracttype'],
  szGenericStr30A: ['mfgcontractidwildcardlookup', 'szgenericstr30a'],
  includeExclude: ['ie', 'includeexclude'],
  userId: ['userid'],
...
const MFC_OUTBOUND_FIELDS = [
  ...
  { localField: 'manufacturerContractId', backendField: 'manufacturerContractId' },
  { localField: 'workStationId', backendField: 'workStationId', useContextFallback: 'workStationId' },
  { localField: 'contractType', backendField: 'contractType' },
  { localField: 'szGenericStr30A', backendField: 'szGenericStr30A' },
...
const MarginFundingContractMaintenanceAddPage = {
  entityName: '',
  gridApi: null,
  gridElement: null,
  ...
  init() {
    this.entityName = String(
      window.MFCC_ENTITY_NAME || 'distribution-fee-contract-attribute'
    ).trim();
    this.cacheDom();
    this.initGrid();
  ...
  initGrid() {
    ...
    this.gridApi = DynamicGrid.createGrid({
      gridElementId: 'mfcContractMaintenanceAddGrid',
    ...
    this.gridElement = document.getElementById('mfcContractMaintenanceAddGrid');
    ...
    setTimeout(() => {
      if (typeof GridManager !== 'undefined' && this.gridApi) {
        GridManager.init(this.gridApi, 'mfcContractMaintenanceAddGrid');
      }
    }, 300);
  },
  ...
  bindToolbarActions() {
    document.addEventListener('click', (event) => {
      ...
      switch (actionButton.dataset.action) {
        case 'back':
          window.location.href = window.MFCC_LIST_PAGE_URL || '/margin-funding-contract-maintenance';
          break;
        case 'delete':
  ...
  getStorageKey() {
    return `margin-funding-contract-jobs:${this.entityName}`;
  },
  ...
  resolveUploadContext() {
    return {
      ...
      programId: String(window.MFCC_PROGRAM_ID || 'PROG001'),
      workStationId: String(window.MFCC_WORK_STATION_ID || 'WS001')
    };
  },
  ...
  ensureToastContainer() {
    let container = document.getElementById('marginFundingContractMaintenanceAddPageToastLayer');
    if (container) return container;

    container = document.createElement('div');
    container.id = 'marginFundingContractMaintenanceAddPageToastLayer';
...
document.addEventListener('DOMContentLoaded', () => {
  MarginFundingContractMaintenanceAddPage.init();
});
