const MCK_PRICE_CAP_FIELD_DEFS = [
  ...
  { field: 'mckBrandPriceChangeCap', headerName: 'MCK Brand Price Change Cap', minWidth: 250, type: 'number' }
];

const MCK_PRICE_CAP_DATE_FIELDS = new Set(['effectiveDate', 'terminationDate']);
const MCK_PRICE_CAP_NUMBER_FIELDS = new Set(['mckBrandPriceChangeCap']);
const MCK_PRICE_CAP_OUTBOUND_FIELDS = [
  { localField: 'terminationDate', backendField: 'terminationDate' },
  { localField: 'mckBrandPriceChangeCap', backendField: 'mckBrandPriceChangeCap' },
  { localField: 'itemCategory', backendField: 'itemCategory' },
  { localField: 'effectiveDate', backendField: 'effectiveDate' }
];

const MckBrandLogicPriceCapAddPage = {
  entityName: '',
  gridApi: null,
  gridElement: null,
  ...
  init() {
    this.entityName = String(window.MCK_PRICE_CAP_ENTITY_NAME || 'gm-core-parameter-mckb-price-change-cap').trim();
    this.cacheDom();
    this.initGrid();
  ...
  cacheDom() {
    ...
    this.uploadStatusInputs = Array.from(document.querySelectorAll('input[name="mckPriceCapUploadStatus"]'));
  ...
  createBlankRow() {
    const row = {};
    MCK_PRICE_CAP_FIELD_DEFS.forEach(({ field }) => {
      row[field] = '';
    });
    row.uploadStatus = '';
    row.uploadErrors = [];
  ...
  initGrid() {
    ...
    this.gridApi = DynamicGrid.createGrid({
      gridElementId: 'mckPriceCapAddGrid',
      ...
      columns: [
        ...
        ...MCK_PRICE_CAP_FIELD_DEFS.map((column) => this.buildColumn(column, dateComparator))
      ]
    });

    this.gridElement = document.getElementById('mckPriceCapAddGrid');
    ...
    if (window.CommunityGridPaste?.attach) {
      const pasteableFields = MCK_PRICE_CAP_FIELD_DEFS.map((column) => column.field);
    ...
    setTimeout(() => {
      if (typeof GridManager !== 'undefined' && this.gridApi) {
        GridManager.init(this.gridApi, 'mckPriceCapAddGrid');
      }
    }, 300);
  },
  ...
  bindToolbarActions() {
    document.addEventListener('click', (event) => {
      ...
      switch (actionButton.dataset.action) {
        case 'back':
          window.location.href = `${window.MCK_BRAND_LOGIC_PAGE_URL || '/manage-mck-brand-logic'}?tab=price-cap`;
          break;
        case 'delete':
  ...
  getStorageKey() {
    return `mck-price-cap-jobs:${this.entityName}`;
  },
  ...
  mapErrorField(field) {
    ...
    const match = MCK_PRICE_CAP_FIELD_DEFS.find((column) => this.normalizeHeader(column.headerName) === normalized || this.normalizeHeader(column.field) === normalized);
    return match?.field || '';
  },
  ...
  getFieldValue(source, field) {
    ...
    const column = MCK_PRICE_CAP_FIELD_DEFS.find((item) => item.field === field);
    if (column) {
  ...
  resolveUploadContext() {
    return {
      ...
      programId: String(window.MCK_PRICE_CAP_PROGRAM_ID || 'PROG001'),
      workStationId: String(window.MCK_PRICE_CAP_WORK_STATION_ID || 'WS001')
    };
  },
  ...
  toBackendRecord(row) {
    const payload = {};
    MCK_PRICE_CAP_OUTBOUND_FIELDS.forEach(({ localField, backendField }) => {
  ...
  resolvePasteHeaderField(header, allowedFields = null) {
    ...
    const match = MCK_PRICE_CAP_FIELD_DEFS.find(({ field, headerName }) => (
  ...
  normalizeRow(row) {
    ...
    MCK_PRICE_CAP_FIELD_DEFS.forEach(({ field }) => {
      ...
      if (MCK_PRICE_CAP_DATE_FIELDS.has(field)) {
        normalized[field] = this.toUsDate(value || '');
      } else if (MCK_PRICE_CAP_NUMBER_FIELDS.has(field)) {
  ...
  isRowEmpty(row) {
    return MCK_PRICE_CAP_FIELD_DEFS.every(({ field }) => !String(row[field] || '').trim());
  },
  ...
  showInfo(message, type = 'success') {
    ...
    let container = document.getElementById('mckPriceCapAddPageToastLayer');
    if (!container) {
      container = document.createElement('div');
      container.id = 'mckPriceCapAddPageToastLayer';
...
document.addEventListener('DOMContentLoaded', () => {
  MckBrandLogicPriceCapAddPage.init();
});
