const MCK_QUALITY_TIER_FIELD_DEFS = [
  ...
  { field: 'qualityScore', headerName: 'Quality Score', minWidth: 180, type: 'number' },
  { field: 'qualityTier', headerName: 'Quality Tier', minWidth: 170 }
];

const MCK_QUALITY_TIER_DATE_FIELDS = new Set(['effectiveDate', 'terminationDate']);
const MCK_QUALITY_TIER_NUMBER_FIELDS = new Set(['itemNum', 'qualityScore']);
const MCK_QUALITY_TIER_OUTBOUND_FIELDS = [
  { localField: 'qualityTier', backendField: 'qualityTier' },
  { localField: 'terminationDate', backendField: 'terminationDate' },
  { localField: 'itemNum', backendField: 'itemNum' },
  { localField: 'itemSubCategory', backendField: 'itemSubCategory' },
  { localField: 'qualityScore', backendField: 'qualityScore' },
  { localField: 'effectiveDate', backendField: 'effectiveDate' }
];

const MckBrandLogicQualityTierAddPage = {
  entityName: '',
  gridApi: null,
  gridElement: null,
  ...
  init() {
    this.entityName = String(window.MCK_QUALITY_TIER_ENTITY_NAME || 'gm-core-parameter-mckb-quality-tiers').trim();
    this.cacheDom();
    this.initGrid();
  ...
  cacheDom() {
    ...
    this.uploadStatusInputs = Array.from(document.querySelectorAll('input[name="mckQualityTierUploadStatus"]'));
  ...
  createBlankRow() {
    const row = {};
    MCK_QUALITY_TIER_FIELD_DEFS.forEach(({ field }) => {
      row[field] = '';
    });
    row.uploadStatus = '';
    row.uploadErrors = [];
  ...
  initGrid() {
    ...
    this.gridApi = DynamicGrid.createGrid({
      gridElementId: 'mckQualityTierAddGrid',
      ...
      columns: [
        ...
        ...MCK_QUALITY_TIER_FIELD_DEFS.map((column) => this.buildColumn(column, dateComparator))
      ]
    });

    this.gridElement = document.getElementById('mckQualityTierAddGrid');
    ...
    if (window.CommunityGridPaste?.attach) {
      const pasteableFields = MCK_QUALITY_TIER_FIELD_DEFS.map((column) => column.field);
    ...
    setTimeout(() => {
      if (typeof GridManager !== 'undefined' && this.gridApi) {
        GridManager.init(this.gridApi, 'mckQualityTierAddGrid');
      }
    }, 300);
  },
  ...
  bindToolbarActions() {
    document.addEventListener('click', (event) => {
      ...
      switch (actionButton.dataset.action) {
        case 'back':
          window.location.href = `${window.MCK_BRAND_LOGIC_PAGE_URL || '/manage-mck-brand-logic'}?tab=quality-tier`;
          break;
        case 'delete':
  ...
  getStorageKey() {
    return `mck-quality-tier-jobs:${this.entityName}`;
  },
  ...
  mapErrorField(field) {
    ...
    const match = MCK_QUALITY_TIER_FIELD_DEFS.find((column) => this.normalizeHeader(column.headerName) === normalized || this.normalizeHeader(column.field) === normalized);
    return match?.field || '';
  },
  ...
  getFieldValue(source, field) {
    ...
    const column = MCK_QUALITY_TIER_FIELD_DEFS.find((item) => item.field === field);
    if (column) {
  ...
  resolveUploadContext() {
    return {
      ...
      programId: String(window.MCK_QUALITY_TIER_PROGRAM_ID || 'PROG001'),
      workStationId: String(window.MCK_QUALITY_TIER_WORK_STATION_ID || 'WS001')
    };
  },
  ...
  toBackendRecord(row) {
    const payload = {};
    MCK_QUALITY_TIER_OUTBOUND_FIELDS.forEach(({ localField, backendField }) => {
  ...
  resolvePasteHeaderField(header, allowedFields = null) {
    ...
    const match = MCK_QUALITY_TIER_FIELD_DEFS.find(({ field, headerName }) => (
  ...
  normalizeRow(row) {
    ...
    MCK_QUALITY_TIER_FIELD_DEFS.forEach(({ field }) => {
      ...
      if (MCK_QUALITY_TIER_DATE_FIELDS.has(field)) {
        normalized[field] = this.toUsDate(value || '');
      } else if (MCK_QUALITY_TIER_NUMBER_FIELDS.has(field)) {
  ...
  isRowEmpty(row) {
    return MCK_QUALITY_TIER_FIELD_DEFS.every(({ field }) => !String(row[field] || '').trim());
  },
  ...
  showInfo(message, type = 'success') {
    ...
    let container = document.getElementById('mckQualityTierAddPageToastLayer');
    if (!container) {
      container = document.createElement('div');
      container.id = 'mckQualityTierAddPageToastLayer';
...
document.addEventListener('DOMContentLoaded', () => {
  MckBrandLogicQualityTierAddPage.init();
});
